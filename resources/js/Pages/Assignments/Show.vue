...
const status = computed(() => {
    return props.assignment.is_actived ? 'Activo' : 'Inactivo';
});
...
<template>
    <Modal>
        <h1 class="text-2xl font-bold text-center">Visualizar asignación</h1>
        <div class="form-group flex flex-col">  
            <div class="mt-4 mb-0">
                <label for="id_driver" class="w-full">Taxista</label>
                <InputText class="w-full" id="id_driver" :placeholder="assignment.drivers.surnames" disabled />
            </div>
            <div class="mt-4 mb-0">
                <label for="id_transport" class="w-full">Transporte</label>
                <InputText class="w-full" id="id_transport" :placeholder="assignment.transports.economic_number" disabled />
            </div>
            <div class="mt-4 mb-0">
            ...
            <div class="w-full flex items-start mt-6 justify-between">
                ...
                <Button label="Borrar" icon="pi pi-trash"
                    ...
                    @click.prevent="destroy(assignment.id)" />
            </div>
        </div>
    </Modal>
</template>
